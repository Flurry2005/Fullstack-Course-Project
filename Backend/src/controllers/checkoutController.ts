...
class CheckoutController {
  async createCheckoutSession(req: Request, res: Response) {
    try {
      ...
      if (!gig) return res.status(400).json({ error: "Gig not found" });
      ...
      switch (tier.toLowerCase()) {
        case "basic":
          if (!gig.basic)
            return res.status(400).json({ error: "Tier doesn't exist" });
      ...
      if (!stripeCustomerId) {
        const customerData: Stripe.CustomerCreateParams = { email: user.email };
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                ...
                images: gig.primaryImagePreview
