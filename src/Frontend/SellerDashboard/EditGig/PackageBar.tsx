import type { Package } from "../../types/Gig";
import { useState, useRef } from "react";
import checkIcon from "../../assets/circle-check-req-icon.svg";
import removeIcon from "../../assets/x-icon.svg";
import deleteIcon from "../../assets/delete-icon-white.svg"

import type { Gig } from "../../types/Gig";
type PackageBarProps = {
  plan: Package;
  name: string;
  editState: boolean;
  setGig: React.Dispatch<React.SetStateAction<Gig | undefined>>;
  gig: Gig;
};

function PackageBar({ plan, name, editState, setGig, gig }: PackageBarProps) {
  const [toggle, setToggle] = useState(false);
  const [featureInput, setFeatureInput] = useState("");
  const [price, setPrice] = useState(plan.price ?? "");
  const featureRef = useRef<HTMLInputElement>(null);

  // Helper to get the correct package key
  const getPackageKey = () => {
    if (name === "Basic") return "basic";
    if (name === "Standard") return "standard";
    if (name === "Premium") return "premium";
    return "basic";
  };

  const addFeature = () => {
    const trimmed = featureInput.trim();
    const pkgKey = getPackageKey();
    const currentFeatures = (gig?.[pkgKey]?.features || []);
    if (trimmed && !currentFeatures.includes(trimmed)) {
      setGig(prev => {
        if (!prev) return prev;
        return {
          ...prev,
          [pkgKey]: {
            ...prev[pkgKey],
            features: [...currentFeatures, trimmed],
          },
        };
      });
      setFeatureInput("");
      featureRef.current?.focus();
    }
  };

  const removeFeature = (idx: number) => {
    const pkgKey = getPackageKey();
    setGig(prev => {
      if (!prev) return prev;
      const currentFeatures = prev[pkgKey]?.features || [];
      return {
        ...prev,
        [pkgKey]: {
          ...prev[pkgKey],
          features: currentFeatures.filter((_, i) => i !== idx),
        },
      };
    });
  };


  return (
    <div className="flex flex-col">
      <div
        className={`flex px-3 items-center flex-wrap  py-3 gap-3 ${toggle && editState
           ? "bg-[#f1f7fc] rounded-t-xl" : "bg-[#F8FAFC] rounded-xl"} border border-[#F1F5F9] ${editState ? "cursor-pointer hover:bg-[#f1f7fc]" : ""}`}
        onClick={() => setToggle((prev) => !prev)}
      >
        {editState &&  (
          <span
            className={`text-xl font-semibold text-[#131B2E] transition-transform duration-100 ${toggle ? "rotate-90" : ""} ease-in-out`}
          >
            {">"}
          </span>
        )}
        <span className="text-xl text-[#131B2E]">{name}</span>
          <span className="ml-auto font-bold text-xl text-[#3525CD]">
            ${plan.price}
          </span>
      </div>
      <div
        className={`${toggle && editState ? "scale-y-100 flex" : "scale-y-0 hidden"} transition-transform duration-100 ease-in-out gap-6 flex-col p-6 border-[#F1F5F9] border-b border-l border-r rounded-b-2xl origin-top`}
        style={{ overflow: "hidden" }}
      >
        <div className="grid grid-cols-1 gap-3 md:flex">
          <div className="flex flex-col gap-1">
            <span className="ml-1">Price ($)</span>
            <input
              type="text"
              className="text-[#6B7280] bg-[#FFFFFF] text-xl p-3 w-20 rounded-lg border border-[#C7C4D8]"
              value={price}
              onChange={e => setPrice(e.target.value)}
              min={0}
            />

        
          </div>
          <div className="flex flex-col gap-1 md:ml-auto">
            <span className=" ml-1">Delivery</span>
            <select
              className="text-[#6B7280] bg-[#FFFFFF] w-fit text-xl p-3 rounded-lg border border-[#C7C4D8]"
              value={plan.delivery}
            >
              <option>1 Day</option>
              <option>2 Days</option>
              <option>3 Days</option>
              <option>5 Days</option>
              <option>1 Week</option>
              <option>10 Days</option>
              <option>2 Weeks</option>
            </select>
          </div>
        </div>
        <div className="flex flex-col gap-1">
        <span className="ml-1 w-full">Included features</span>
        <div className="flex flex-col gap-2">
          {(plan.features || []).map((feature, id) => (
            <span key={id} className="flex min-w-0 items-center gap-2">
              <img src={checkIcon} className="w-5 h-5" alt="check" />
              <span className="break-all leading-tight">{feature}</span>
              <img
                src={removeIcon}
                onClick={() => removeFeature(id)}
                className="cursor-pointer contrast-100 invert-50 w-3 h-3"
                alt="remove"
              />
            </span>
          ))}
          </div>
        </div>
        <div className="flex flex-col flex-wrap gap-3 mt-2">
          <input
            type="text"
            ref={featureRef}
            className="text-[#6B7280] bg-[#FFFFFF] text-xl p-3 w-full rounded-lg border border-[#C7C4D8] flex-1"
            value={featureInput}
            onChange={e => setFeatureInput(e.target.value)}
            placeholder="Add a feature..."
          />
          <button
            onClick={addFeature}
            className="cursor-pointer py-1 rounded-lg md:w-fit font-semibold text-white bg-[#4F46E5] px-6"
            type="button"
          >
            Add
          </button>
        </div>

        <button disabled={name === "Basic"} className={`${name === "Basic" ? "hidden" : ""} ml-auto w-fit cursor-pointer py-3 rounded-lg font-semibold  border border-neutral-500/15 text-white bg-red-400 px-6 flex items-center gap-1`}><img src={deleteIcon} className="w-6 h-6"/>Remove Plan</button>
      
      </div>
    </div>
  );
}

export default PackageBar;
